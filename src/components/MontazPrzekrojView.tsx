...
import { przekrojObuRezimow, rolaZeSposobu, udzialPct, type KolumnaMontazu, type WejscieMontazu } from "@/lib/finanse/przekroj";
...
/**
 * Wynik M3 — przekrój montażu, OBA reżimy obok siebie. Nie werdykt: koszt inwestycji
 * rozbity na źródła, wkład własny jako pozycja domykająca. Suwak kosztu budowy
 * przelicza cały montaż w obu reżimach na żywo.
 */
export function MontazPrzekrojView({
  ...
  const przekroj = useMemo(() => {
    const wej: WejscieMontazu = {
      ...
      pumCalkowiteM2: wariant.pumM2 + wariant.powWspolneUslugoweM2,
      kosztBudowyM2,
      wartoscOdtworzeniowaM2,
      wartoscDzialkiPln: profil.wartoscDzialkiPln ?? 0,
      rolaDzialki: rola,
      partycypacjaNajemcowPct: profil.partycypacjaNajemcowPct ?? 0,
      wkladGminyPct: profil.wkladGminyPct ?? 0,
    };
    return przekrojObuRezimow(wej);
  }, [wariant, wartoscOdtworzeniowaM2, profil, kosztBudowyM2, rola]);

  return (
    <div className="space-y-4">
      {/* Suwak kosztu budowy */}
      <Karta tytul="Koszt budowy — dźwignia montażu" podtytul={`zł/m² × PUM (${Math.round(wariant.pumM2 + wariant.powWspolneUslugoweM2)} m²) · przelicza oba reżimy na żywo`}>
      ...
      <div className="grid md:grid-cols-2 gap-4 items-start">
        <KolumnaRezimu k={przekroj.obecny} rola={rola} etykietaWniesienia={ETYK_GRUNTU[profil.sposobWniesieniaDzialki]} />
        <KolumnaRezimu k={przekroj.przyszly} rola={rola} etykietaWniesienia={ETYK_GRUNTU[profil.sposobWniesieniaDzialki]} />
      </div>
...
function KolumnaRezimu({ k, rola, etykietaWniesienia }: { k: KolumnaMontazu; rola: string; etykietaWniesienia: string }) {
    ...
    <div className="rounded-card border border-grunt-border overflow-hidden">
      ...
      <div className="p-4 space-y-3">
        ...
        <div>
          ...
          <Pozycja label="Grant / dotacja" kwota={k.zrodla.grant} razem={razem} />
          ...
          {k.zrodla.wkladGminy > 0 && <Pozycja label="Wkład gminy" kwota={k.zrodla.wkladGminy} razem={razem} />}
        </div>
        ...
        <div className="rounded-md bg-grunt-ink/5 border border-grunt-ink/20 px-3 py-2.5">
          <div className="flex items-baseline justify-between">
            <span className="text-[12px] font-semibold text-grunt-text">Wkład własny (domyka)</span>
        ...
        <details className="text-[11px] text-grunt-text-muted2">
          ...
          <div className="mt-1.5 grid grid-cols-2 gap-x-3 gap-y-1">
            ...
            <span>Grant do {k.zalozenia.maxGrantPct}%</span>
